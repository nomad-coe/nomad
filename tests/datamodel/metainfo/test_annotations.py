...
from nomad.datamodel.metainfo.annotations import PlotAnnotation, ELNAnnotation, PlotlyGraphObjectAnnotation
...
@pytest.mark.parametrize('annotation, result, error_type, error', [
...
def test_plotly_graph_object_validation(annotation, result, error_type, error):
    if error:
        ...
        assert PlotlyGraphObjectAnnotation(**annotation).dict(exclude_none=True) == (result or annotation)
