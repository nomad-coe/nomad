...
from nomad.metainfo import Package, MSection, Quantity, Reference, SectionProxy, SubSection
...
class Holder(MSection):
    base = Quantity(
        type=Reference(SectionProxy('Base')),
        shape=[])

    derived = Quantity(
        type=Reference(SectionProxy('Derived')),
        shape=[])
