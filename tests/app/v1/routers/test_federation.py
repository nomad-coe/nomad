...
@pytest.mark.parametrize('is_gzip', [
    pytest.param(True, id='with gzip'),
...
def test_single_logrecord(is_gzip, api_v1, central_logstash_mock):
    ...
    assert json.dumps(log_sent_json, sort_keys=True) == json.dumps(log_received_json, sort_keys=True)


@pytest.mark.parametrize('send_header_ip,expected_ip', [
    pytest.param('127.0.0.1', '127.0.0.1', id='single ip'),
    pytest.param('127.0.0.1, 127.99.99.99', '127.0.0.1', id='concatenate ip')
])
@pytest.mark.timeout(3)
def test_nginx_ipaddress_header(send_header_ip, expected_ip, api_v1, central_logstash_mock):
