...
def test_porosity(filepath):
    ...
    topology_porous = [top for top in archive.results.material.topology if top.method == 'porosity']
...
def test_mof(filepath):
    ...
    topology_porous = [top for top in archive.results.material.topology if top.method == 'porosity']
    porous_system = topology_porous[0]
    metal_sbus = [m_sbu.label for m_sbu in topology_porous[1:]
                  if 'metal_sbu' in m_sbu.label]
    organic_sbus = [m_sbu.label for m_sbu in topology_porous[1:]
                    if 'organic_sbu' in m_sbu.label]
    ligands = [m_sbu.label for m_sbu in topology_porous[1:]
               if 'organic_ligand' in m_sbu.label]
    assert porous_system.largest_cavity_diameter != 0
    assert porous_system.pore_limiting_diameter != 0
    assert porous_system.largest_included_sphere_along_free_sphere_path != 0
    assert porous_system.accessible_surface_area != 0
