...
def generate_metainfo_code(metainfo_pkg: Package, python_package_path: str):
    ...
    def format_description(description, indent=0, width=90):
        ...
        def format_paragraph(paragraph, first):
            ...
            return textwrap.indent(
                '\n'.join(lines),
                ' ' * 4 * indent,
                lambda x: not (first and x.startswith(lines[0])),
            )

        return '\n\n'.join(
            [format_paragraph(p, i == 0) for i, p in enumerate(paragraphs) if p != '']
        )
    ...
    def format_type(pkg, mi_type):
        ...
        if isinstance(mi_type, Reference):
            if pkg == mi_type.target_section_def.m_parent:
                ...
                python_module = (
                    mi_type.target_section_def.m_parent.a_legacy.python_module
                )
                return '%s.%s' % (
                    python_module.split('.')[-1],
                    mi_type.target_section_def.name,
                )
    ...
    def format_definition_refs(pkg, definitions):
        ...
        return ', '.join(
            [format_definition_ref(definition) for definition in definitions]
        )
    ...
    with open(python_package_path, 'wt') as f:
        ...
        code = '\n'.join(
            [line.rstrip() if line.strip() != '' else '' for line in code.split('\n')]
        )
        f.write(code)
...
if __name__ == '__main__':
    ...
    from nomad.datamodel.metainfo.system import m_package

    generate_metainfo_code(m_package, 'nomad/datamodel/metainfo/system.py')
