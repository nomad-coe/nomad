...
def api_url(
    ssl: bool = True, api: str = 'api', api_host: str = None, api_port: int = None
):
    """
...
def north_url(ssl: bool = True):
    return api_url(
        ssl=ssl, api='north', api_host=north.hub_host, api_port=north.hub_port
    )
    ...
    options={
        ...
        'parsers/castep': Parser(
            ...
            mainfile_contents_re=(
                r'\s\|\s*CCC\s*AA\s*SSS\s*TTTTT\s*EEEEE\s*PPPP\s*\|\s*'
            ),
        ),
        ...
        'parsers/gaussian': Parser(
            ...
            mainfile_mime_re=r'.*',
            mainfile_contents_re=(
                r'\s*Cite this work as:' r'\s*Gaussian [0-9]+, Revision [A-Za-z0-9\.]*,'
            ),
        ),
        'parsers/gpaw': Parser(
        ...
        'parsers/openmx': Parser(
            ...
            mainfile_name_re=r'.*\.out$',
            mainfile_contents_re=(
                r'^\*{59}\s+\*{59}\s+This calculation was performed by OpenMX'
            ),
        ),
        'parsers/orca': Parser(
        ...
        'parsers/quantumespresso': Parser(
            ...
            mainfile_contents_re=(
                r'(Program PWSCF.*starts)|(Current dimensions of program PWSCF are)'
            ),
            supported_compressions=['gz', 'bz2', 'xz'],
        ),
        ...
        'parsers/asap': Parser(
            ...
            mainfile_mime_re=r'application/octet-stream',
            mainfile_binary_header_re=rb'AFFormatASE\-Trajectory',
        ),
        ...
        'parsers/phonopy': Parser(
            ...
            mainfile_name_re=(
                r'(.*/phonopy-FHI-aims-displacement-0*1/control.in$)|(.*/phon[^/]+yaml)'
            ),
        ),
        ...
        'parsers/w2dynamics': Parser(
            ...
            mainfile_contents_dict={
                '__has_all_keys': ['.axes', '.config', '.quantities']
            },
        ),
        ...
        'parsers/ocean': Parser(
            ...
            mainfile_mime_re=r'(application/.*)|(text/.*)',
            mainfile_contents_dict={
                '__has_all_keys': ['bse', 'structure', 'screen', 'calc']
            },
        ),
        ...
        'parsers/elabftw/elabftw': Parser(
            python_package='nomad.parsing.elabftw', parser_as_interface=True
        ),
        'parsers/chemotion/chemotion': Parser(
            python_package='nomad.parsing.chemotion', parser_as_interface=True
        ),
...
def _merge(
    a: Union[dict, BaseModel, None], b: Union[dict, BaseModel, None]
    ...
    # If original value is None, it should be overwritten by new value
        ...
        for key in b.__dict__ if isinstance(b, BaseModel) else b:
            ...
            if has(a, key):
                child = get(a, key)
                # Objects are merged
                if isinstance(value, (BaseModel, dict)) or isinstance(
                    child, (BaseModel, dict)
                ):
...
def _apply(key, value, raise_error: bool = True) -> None:
    ...
    if group_key not in current:
        ...
        try:
            ...
            current_type = (
                type_hints[config_key]
                if config_key in type_hints
                else type(current_value)
            )
        except AttributeError:
            if raise_error:
                logger.error(f'config key does not exist: {full_key}')
