...
async function testMaterialMethod(repo, archive) {
  ...
  expect(within(structural_type).getByText(repo.dft.system)).toBeInTheDocument()
  ...
  expect(within(space_group).getByText(`${repo.dft.spacegroup_symbol} (${repo.dft.spacegroup})`)).toBeInTheDocument()
...
test('correctly renders method and material data with new index', async () => {
  await testMaterialMethod(repoDftBulk, archiveDftBulk)
})
test('correctly renders method and material data with old index', async () => {
  await testMaterialMethod(repoDftBulkOld, archiveDftBulkOld)
})
...
async function testElectronicProperties(repo) {
  renderWithAPIRouter(
    <DFTEntryOverview
      data={repo}
    ></DFTEntryOverview>
...
test('correctly renders electronic properties with new index', async () => {
  await testElectronicProperties(repoDftBulk)
})

test('correctly renders electronic properties with old index', async () => {
  await testElectronicProperties(repoDftBulkOld)
})

async function testVibrationalProperties(repo) {
  renderWithAPIRouter(
    <DFTEntryOverview
      data={repo}
    ></DFTEntryOverview>
...
test('correctly renders vibrational properties with new index', async () => {
  await testVibrationalProperties(repoDftBulk)
})

test('correctly renders vibrational properties with old index', async () => {
  await testVibrationalProperties(repoDftBulkOld)
})
